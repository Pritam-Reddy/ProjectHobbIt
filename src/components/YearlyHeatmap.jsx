  ...
  return (
    <div className="w-full space-y-2">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end px-1 gap-2 sm:gap-0 mb-4">
        <div>
          <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200">{totalContributions} contributions in {year}</h4>
      ...
      <div ref={scrollRef} className="w-full overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700 scrollbar-track-transparent">
        <div className="flex flex-col min-w-max">
          <div className="flex h-4 mb-1 relative text-[9px] font-bold text-slate-400 dark:text-slate-500 pl-8">
             {months.map((m, i) => (
                <div key={m.name} style={{ position: 'absolute', left: `${m.index * 14}px` }}>{m.name}</div>
             ))}
          </div>
              ...
              {weeks.map((week, weekIdx) => (
                <div key={weekIdx} className="grid grid-rows-7 gap-1">
                  {week.map((day, dayIdx) => {
                    ...
                    return (
                      <div key={day.dateStr} className={`w-2.5 h-2.5 rounded-[2px] transition-colors duration-200 relative group cursor-pointer ${getColor(day.intensity)}`}>
                        {day.intensity !== -1 && (
