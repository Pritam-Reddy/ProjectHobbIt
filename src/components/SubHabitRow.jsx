...
const SubHabitRow = ({ name, subHabit, days, completedDates, onToggle, onDelete, onEdit, sidebarWidth, isParentScheduled, subValues, onUpdateSubValue, subNotes, onOpenNote }) => {
  ...
  const isScheduled = (date) => {
    ...
    if (subHabit.frequency) return subHabit.frequency.includes(format(date, 'EEE'));
    return true;
  };
  ...
  const handleClick = (day) => {
    ...
    if (isQuantitative) {
      ...
      if (input !== null) onUpdateSubValue(dateStr, input);
    } else {
      onToggle(day);
    }
  };
  ...
  return (
    <div className="flex border-b border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-900/40 hover:bg-slate-50/70 dark:hover:bg-slate-800/60 transition-colors group/sub">
      <div className="px-2 md:px-4 py-2 md:py-3 pl-8 md:pl-12 flex items-center gap-2 shrink-0 sticky left-0 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 z-20 shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] dark:shadow-none transition-colors" style={{ width: `${sidebarWidth}px` }}>
        <span className="text-slate-500 dark:text-slate-400 text-xs md:text-sm font-medium truncate select-none flex-1 cursor-pointer hover:underline" onClick={onEdit}>└ {name}</span>
        {isQuantitative && <span className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-bold mr-2">Goal: {subHabit.goal} {subHabit.unit}</span>}
        <div className="flex gap-1 opacity-0 group-hover/sub:opacity-100 transition-opacity">
           <button onClick={onEdit} className="p-1 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded"><Pencil size={12} className="md:w-3.5 md:h-3.5" /></button>
           <button onClick={onDelete} className="p-1 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded"><Trash2 size={12} className="md:w-3.5 md:h-3.5" /></button>
        </div>
      </div>
      ...
      <div className="flex">
        {days.map((day) => {
          ...
          const hasNote = subNotes[dateStr] && subNotes[dateStr].trim().length > 0;
